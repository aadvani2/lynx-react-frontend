...
export interface AcceptedRequestDetails {
  request: {
    ...
    created_at: string;
    updated_at: string;
    type: string; // Moved from AcceptedRequestDetails
...
interface AcceptedRequestDetailsState {
  // State
  requestDetails: AcceptedRequestDetails | null;
  loading: boolean;
  error: string | null;
  ...
  // Actions
  setRequestDetails: (details: AcceptedRequestDetails | null) => void;
...
export const useAcceptedRequestDetailsStore = create<AcceptedRequestDetailsState>((set, get) => ({
  ...initialState,
  ...
  fetchScheduleHistory: async (requestId: number, userTimezone: number) => {
    ...
    try {
      ...
      // Handle the response structure: {"success":true,"data":{"history":[]}}
